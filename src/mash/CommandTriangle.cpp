...
int CommandTriangle::run() const
{
    ...
    {
        threadPool.runWhenThreadAvailable(new TriangleInput(sketch, i, parameters, distanceMax, pValueMax, emitJaccard));
...
CommandTriangle::TriangleOutput * compare(CommandTriangle::TriangleInput * input)
{
    ...
    {
        compareSketches(&output->pairs[i], sketch.getReference(input->index), sketch.getReference(i), sketchSize, sketch.getKmerSize(), sketch.getKmerSpace(), input->maxDistance, input->maxPValue, input->emitJaccard);
    }
